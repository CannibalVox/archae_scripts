class_name UpdatedTrenchBroomGameConfigFile
extends Resource
tool
...
export(Array, Resource) var compilation_tools : Array = []
...
func build_class_text() -> String:
	...
	var compilation_tools_str = parse_compilation_tools(compilation_tools)
...
func parse_compilation_tools(compilationTools: Array) -> String:
	var tools_str := ""
	
	for compilationTool in compilationTools:
		tools_str += "{\n"
		tools_str += "\t\t\t\"name\": \"%s\",\n" % compilationTool.name 
		tools_str += "\t\t\t\"description\": \"%s\"\n" % compilationTool.description 
		tools_str += "\t\t}"
		
		if compilationTool != compilationTools[-1]:
			tools_str += ","
